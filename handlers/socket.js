...
module.exports = {
  networkRttStat: (socket, data) => socket.emit("rtt-pong", data),
        ...
        Chat.findOne({ _id: to, "members._id": userId, type: "room" })
          .then((chat) => {
            if (chat) {
              ...
              ErrorHandlers.msg(socket.id, "Salon introuvable.");
            }
          })
          .catch((err) => {
            ErrorHandlers.err(err, socket.id);
    ...
    switch (type) {
      case "direct":
        for (let i = 0; i < 20; i++) {
          ...
          socket.emit("rings", {
            status:
              serverStore.getActiveConnections(target).length > 0
                ? "connected"
                : "not connected",
          });
        }
  ...
  callHandler: async (socket, data) => {
    ...
    switch (type) {
      ...
      case "room":
        roomDetails = await Chat.findOne(
          { _id: who, "members._id": userId, type: "room" },
    ...
    const userDetails = await User.findById(
      userId,
      "_id fname mname lname email imageUrl"
    );
    // message = new Message({
    ...
    message
      ...
      .then(() => {
        ...
        Chat.findOne({ type: type })
          .or([{ _id: who }, { "members._id": { $all: [userId, who[0]] } }])
          .then((chat) => {
            if (chat) {
              chat.messages.push(message._id);
              chat
  ...
  pickUpHandler: async (socket, data) => {
    ...
    const userDetails = await User.findById(
      userId,
      "_id fname mname lname email imageUrl contacts"
    );
    ...
    switch (type) {
      ...
      case "room":
        const roomDetails = await Chat.findOne(
          { _id: from, "members._id": userId, type: "room" },
          { "members._id": 1 }
        );
        if (!roomDetails) {
      ...
      case "direct":
        ...
        if (status != null) {
          let chatId = await Chat.findOne(
            { "members._id": { $all: [from, userId] }, type: "direct" },
            { _id: 1 }
          );
          chatId = chatId._id.toString();
          callDetails.status =
            status === 1 ? 2 : details.response === "rejected" ? 4 : 3;
          callDetails
            .save()
            .then(() => {
    ...
    switch (type) {
      case "direct":
        ...
        if (!userDetails.contacts.includes(to)) {
          ErrorHandlers.msg(
            socket.id,
            "La cible ne figure pas dans vos contacts."
          );
        }
      ...
      case "room":
        const roomDetails = await Chat.findOne(
          { _id: to, "members._id": userId, type: "room" },
          { "members._id": 1 }
        );
        roomDetails.members.forEach((member) => {
          const memberId = member._id;
