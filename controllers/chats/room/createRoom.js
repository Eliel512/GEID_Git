  ...
  participants.forEach(() => {
    ...
    do {
      ...
      randomNumbers.includes(randomNumber) |
        screenNumbers.includes(randomNumber) &&
      randomNumbers.includes(screenNumber) |
        screenNumbers.includes(screenNumber)
    );
    ...
    case "direct":
      ...
      if (!userDetails.contacts.includes(target)) {
        return {
          error: true,
          message:
            "La cible ne figure pas dans la liste des contacts de l'utilisateur",
        };
      }
      break;
    case "room":
...
module.exports = async (req, res, next) => {
  ...
  //console.log("LOG:", "userDetails => ", userDetails);
  let roomId;
  let roomDetails;
  let query;
  ...
  switch (data.type) {
    case "direct":
      ...
      query = {
        "members._id": { $all: [data.target, userId] },
        type: "direct",
      };
      break;
    ...
    default:
      return res
        .status(400)
        .json({ message: "Le parametre 'type' est incorrect" });
  }

  try {
    roomDetails = await Chat.findOne(query, { messages: 0, __v: 0 })
      .populate({
        path: "members._id",
        model: User,
    ...
    participants: roomDetails.members.map((participant) => {
      return {
        ...
        state: {
          ...
          screenShared: false,
          isMicActive:
            participant._id._id == userId
              ? Boolean(state.isMicActive)
              : undefined,
          isCamActive:
            participant._id._id == userId
              ? Boolean(state.isCamActive)
              : undefined,
  ...
  callSessionObject
    ...
    .then(async () => {
      ...
      if ((data.type == "room" && !data.scheduled) || data.type == "direct") {
        ...
        //     location:
        //       data.type === "direct" ? data.target : callSessionObject.location,
        //   },
        // });
      }
      ...
      callSessionObject.participants.forEach((member) => {
        if (
          member.identity._id != userId ||
          (data.scheduled && data.type == "room")
        ) {
          receiverList.push(
            ...roomStore.getActiveConnections(member.identity._id)
          );
        }
      });
      ...
      if (receiverList.length == 0 && data.type == "direct") {
        ...
        const eventToEmit =
          data.type == "room" && data.scheduled ? "scheduled" : "call";
