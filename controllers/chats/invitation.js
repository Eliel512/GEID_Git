...
module.exports = {
  sendInvite: async (req, res) => {
    ...
    try {
      ...
      if (targetMail === userEmail.email) {
        return res.status(409).json({
          message:
            "Vous ne pouvez pas envoyé d'invitation à votre propre adresse.",
        });
      }
    } catch (error) {
      console.log(error);
    ...
    if (!objectsList.includes(object.toLowerCase())) {
      return res.status(400).json({
        message: "L'objet de linvitation est soit 'connexion' soit 'salon'.",
      });
    }
    ...
    User.findOne({ email: targetMail })
      .then(async (targetUser) => {
        ...
        if (isInvitationAlreadySent) {
          return res.status(409).json({
            message:
              "Vous avez déjà envoyé une invitation à l'utilisateur spécifié.",
          });
        }
    ...
    if (invitationExists) {
      Invitation.deleteOne({ _id: _id })
        .then(() => {
          res
            .status(200)
            .json({ message: "Invitation supprimée avec succès!" });
    ...
    Invitation.findOne({ _id: _id, to: receiverId })
      .then(async (invitation) => {
        ...
        try {
          ...
          //updatesHandler.updatePendingInvitations(to, _id);
          updatesHandler.updateContacts(from, { to });
          updatesHandler.updateContacts(to, { from });
      ...
      .catch(() =>
        res
          .status(400)
          .json({ message: "Coordonnées de l'invitation invalides." })
      );
  },
  getInvite: async (req, res) => {
    const invits = await Invitation.find({ to: res.locals.userId }).populate(
      "from",
      "_id email fname mname lname grade.grade grade.role",
      User
    );
